...
function LineItem(props) {
	function click(event) {
		...
		document.getElementById(`select-menu-${props.num}`).style.display =
			"none";
	}

	function openClose() {
		let display =
			document.getElementById(`select-menu-${props.num}`).style
				.display === "flex"
				? "none"
				: "flex";
		document.getElementById(
			`select-menu-${props.num}`,
		).style.display = display;
		if (display === "flex")
			document.getElementsByClassName("better-select")[props.num].focus();
	}
	...
	return (
		<div className="line-item">
			...
			<div className="ddmenu">
				<button className="ddbutton" type="button" onClick={openClose}>
					{props.lineItem.multiplier}
				</button>
				<div tabindex={0} className="better-select">
					<div
						id={`select-menu-${props.num}`}
...
export default class Calculator extends Component {
		...
		for (let i = 0; i < this.props.lineItems.length; i++) {
			arr.push(
				<LineItem
					...
					showDeleteButton={
						this.props.lineItems.length > 1 ? true : false
					}
				/>,
			);
		}
	...
	render() {
		return <form id="calculator">{this.renderLineItems()}</form>;
	}
}
